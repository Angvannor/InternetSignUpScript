...
所以工具每次运行都会把 `wlanuserip` 和 `ip` 换成**认证服务器看到的那个 IP**。

判断依据有两级，优先用更可靠的那个：

1. **门户页面里报告的客户端 IP** —— `a70.htm` 里有一行 `v46ip='10.53.53.219';`
   和 `ss5="10.53.53.219";`，这就是**认证服务器眼里你的 IP**。
   它是最准的，用 `--probe` 能看到。
2. 本机网卡 IP（用一个不发包的 UDP connect 让系统选路由得到）。

**为什么必须优先用第 1 个**：如果宿舍里放了路由器做 NAT，
你电脑的网卡 IP 和"服务器看到的 IP"根本不是同一个，此时提交网卡 IP 必然认证失败。
`--probe` 会把两个 IP 都打出来，不一致时会明确提示：

```
本机用于访问门户的 IP：10.53.60.27
认证服务器看到的客户端 IP：10.16.27.9
两者不一致！… 说明你和认证服务器之间还有一层路由器/NAT。
提交给门户的 wlanuserip 必须是服务器看到的那个，本工具已按此处理。
```

**Q：怎么确认我输的密码没被输入法搞坏？**

初始化时工具会做一次"密码形状自检"，只报告形状、**绝不打印密码内容**：

```
密码自检：长度=8 字符，全部为半角 ASCII
```

如果看到下面这类警告，那基本就是登录失败的原因：

```
⚠ 密码警告：含 3 个全角字符：中文输入法下打出的 ＡＢＣ１２３ 和半角的 ABC123 看起来一样但并不相等
